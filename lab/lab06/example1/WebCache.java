...
public class WebCache {
	...
	public String requestURL(String url) {
		...
		if (cache.containsKey(url)) { // daca deja avem in cache continutul
			System.out.println("The url was found in the cache with the frequency: " + ((WebCacheEntry)cache.get(url)).getFreq());
		...
		System.out.println("The url wasn't found in the cache.");
		...
		if (maxSizeOnDisk > 0l) {
			...
			while (sizeOnDisk > maxSizeOnDisk) {
				System.out.println("Too much space used, remove files.");
				// cat timp am depasit mai stergem intrari
				...
				for (Enumeration en = cache.keys(); en.hasMoreElements(); ) {
					...
					if (wce == null) {
						...
						if (wce.getFreq() > e.getFreq())
							wce = e; // alegem pagina cu cea mai mica frecventa (LFU)
					}
				}

				if (wce == null) break;
				// stergem respectiva intrare din cache
				System.out.println("Remove: " + wce.getURL());
